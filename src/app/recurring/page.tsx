  ...
  return (
    <div className="space-y-6">
      ...
      {accounts.length === 0 ? (
        ...
        <form action={createRecurring} className="grid grid-cols-1 gap-3 rounded-2xl border border-black/10 dark:border-white/10 bg-black/[0.03] dark:bg-white/5 p-4 sm:grid-cols-3">
          ...
          <label className="flex flex-col text-sm">
            ...
            <select name="frequency" className="rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/30 px-3 py-2">
          ...
          <label className="flex flex-col text-sm">
            ...
            <select name="accountId" required className="rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/30 px-3 py-2">
              {accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
            </select>
          </label>
          <label className="flex flex-col text-sm">
            <span className="mb-1 text-gray-500 dark:text-gray-400">Đến tài khoản (khi chuyển)</span>
            <select name="toAccountId" className="rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/30 px-3 py-2">
              <option value="">—</option>
              {accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
            </select>
          </label>
          <label className="flex flex-col text-sm">
            ...
            <select name="categoryId" className="rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/30 px-3 py-2">
              <option value="">—</option>
              {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
          </label>
          <label className="flex flex-col text-sm">
            <span className="mb-1 text-gray-500 dark:text-gray-400">Bắt đầu</span>
            <input name="startDate" type="date" required className="rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/30 px-3 py-2" />
          </label>
          <label className="flex flex-col text-sm">
            <span className="mb-1 text-gray-500 dark:text-gray-400">Kết thúc (tuỳ chọn)</span>
            <input name="endDate" type="date" className="rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/30 px-3 py-2" />
          </label>
          <input name="note" placeholder="Ghi chú" className="rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/30 px-3 py-2 sm:col-span-2" />
      ...
      <div className="space-y-2">
        {items.length === 0 && <p className="text-gray-500 dark:text-gray-400">Chưa có giao dịch định kỳ nào.</p>}
        {items.map((r) => (
          <div
            key={r.id}
            className={`rounded-xl border border-black/10 dark:border-white/10 bg-black/[0.03] dark:bg-white/5 px-4 py-3 ${!r.active ? "opacity-60" : ""}`}
          >
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <form action={toggleRecurring}>
                  ...
                  <button className="rounded-lg border border-black/15 dark:border-white/15 px-2.5 py-1 text-xs hover:bg-black/5 dark:hover:bg-white/10">
            ...
            <details className="mt-2">
              ...
              <form action={updateRecurring} className="mt-2 flex flex-wrap items-end gap-2">
                ...
                <label className="flex flex-col text-xs">
                  <span className="mb-1 text-gray-500 dark:text-gray-400">Tần suất</span>
                  <select name="frequency" defaultValue={r.frequency} className="rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/30 px-2 py-1">
                    <option value="MONTHLY">Hằng tháng</option>
                    <option value="WEEKLY">Hằng tuần</option>
                    <option value="DAILY">Hằng ngày</option>
                  </select>
                ...
                <label className="flex flex-col text-xs">
                  <span className="mb-1 text-gray-500 dark:text-gray-400">Kỳ tới</span>
                  <input name="nextRun" type="date" defaultValue={toInputDate(r.nextRun)} className="rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/30 px-2 py-1" />
                </label>
                <label className="flex flex-col text-xs">
                  <span className="mb-1 text-gray-500 dark:text-gray-400">Kết thúc</span>
                  <input name="endDate" type="date" defaultValue={r.endDate ? toInputDate(r.endDate) : ""} className="rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/30 px-2 py-1" />
                </label>
                <label className="flex flex-col text-xs">
                  <span className="mb-1 text-gray-500 dark:text-gray-400">Ghi chú</span>
                  <input name="note" defaultValue={r.note ?? ""} className="w-40 rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/30 px-2 py-1" />
                </label>
