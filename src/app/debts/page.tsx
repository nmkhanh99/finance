    ...
    <div className="space-y-8">
      ...
      >
        <label className="col-span-2 flex flex-col text-sm">
          <span className="mb-1 text-gray-400">Tên khoản vay</span>
          <input
            name="name"
            required
        ...
        <label className="flex flex-col text-sm">
          <span className="mb-1 text-gray-400">Gốc (VND)</span>
          <input name="principal" type="number" step="1000" min="0" required className="rounded-lg border border-white/10 bg-black/30 px-3 py-2" />
        </label>
        <label className="flex flex-col text-sm">
          <span className="mb-1 text-gray-400">Lãi suất (%/năm)</span>
          <input name="ratePercent" type="number" step="0.1" min="0" required className="rounded-lg border border-white/10 bg-black/30 px-3 py-2" />
        </label>
        <label className="flex flex-col text-sm">
          <span className="mb-1 text-gray-400">Kỳ hạn (tháng)</span>
          <input name="termMonths" type="number" step="1" min="1" required className="rounded-lg border border-white/10 bg-black/30 px-3 py-2" />
        </label>
        <label className="flex flex-col text-sm">
          <span className="mb-1 text-gray-400">Loại lãi</span>
          <select name="interestType" className="rounded-lg border border-white/10 bg-black/30 px-3 py-2">
        ...
        <label className="col-span-2 flex flex-col text-sm">
          <span className="mb-1 text-gray-400">Ngày bắt đầu</span>
          <input name="startDate" type="date" className="rounded-lg border border-white/10 bg-black/30 px-3 py-2" />
        </label>
      ...
      <div className="space-y-4">
        {debts.length === 0 && <p className="text-gray-400">Chưa có khoản vay nào.</p>}
        {debts.map((d) => {
          ...
          return (
            <div key={d.id} className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
                <div><div className="text-gray-400">Dư nợ còn lại</div><div className="font-semibold text-red-400">{formatMoney(outstanding)}</div></div>
                <div><div className="text-gray-400">Trả/tháng (ước tính)</div><div className="font-semibold">{formatMoney(s.monthly)}</div></div>
                <div><div className="text-gray-400">Tổng lãi cả kỳ</div><div className="font-semibold text-amber-400">{formatMoney(s.lifeInterest)}</div></div>
                <div><div className="text-gray-400">Đã trả (gốc / lãi)</div><div className="font-semibold">{formatMoney(paidPrincipal)} / {formatMoney(paidInterest)}</div></div>
              </div>

              {/* Ghi nhận trả nợ */}
              <form action={addPayment} className="mt-4 flex flex-wrap items-end gap-2 border-t border-white/10 pt-4">
                <input type="hidden" name="debtId" value={d.id} />
                <label className="flex flex-col text-xs">
                  <span className="mb-1 text-gray-400">Số tiền trả</span>
                  <input name="amount" type="number" step="1000" min="0" required className="w-40 rounded-lg border border-white/10 bg-black/30 px-3 py-1.5" />
                </label>
                <label className="flex flex-col text-xs">
                  <span className="mb-1 text-gray-400">Ngày</span>
                  <input name="date" type="date" className="rounded-lg border border-white/10 bg-black/30 px-3 py-1.5" />
                </label>
                ...
                <span className="text-xs text-gray-500">(tự tách lãi theo dư nợ, phần còn lại trừ vào gốc)</span>
              </form>
                ...
                <details className="mt-4">
                  ...
                  <div className="mt-2 max-h-64 overflow-y-auto">
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-[#0b0f1a] text-left text-gray-400">
