  ...
  function List({ items, title }: { items: typeof categories; title: string }) {
    return (
      <div className="space-y-2">
        ...
        {items.length === 0 && <p className="text-sm text-gray-500 dark:text-gray-400">Chưa có danh mục.</p>}
        {items.map((c) => (
          <div
            key={c.id}
            className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-black/10 dark:border-white/10 bg-black/[0.03] dark:bg-white/5 px-4 py-2"
          >
            <form action={renameCategory} className="flex items-center gap-2">
              ...
              <input
                name="name"
                defaultValue={c.name}
                className="w-44 rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/30 px-3 py-1.5 text-sm"
              />
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs text-gray-600 dark:text-gray-500">{c._count.transactions} giao dịch</span>
    ...
    <div className="space-y-8">
      ...
      >
        <label className="flex flex-col text-sm">
          <span className="mb-1 text-gray-500 dark:text-gray-400">Tên danh mục</span>
          <input name="name" required placeholder="VD: Cà phê" className="w-56 rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/30 px-3 py-2" />
        </label>
        <label className="flex flex-col text-sm">
