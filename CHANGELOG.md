...
### Added
- **Mở rộng mã crypto + CoinGecko ID tùy chỉnh**: danh sách mã hỗ trợ sẵn tăng 20 → ~50 (thêm ATOM/ARB/OP/SUI/TIA/INJ...). Mỗi holding nhập được **CoinGecko ID** riêng (vd `bitcoin`) ở form Đầu tư → cập nhật giá **bất kỳ coin nào**, ưu tiên hơn map theo symbol. `resolveCoinId` (+4 test); `Holding.priceId` (migration `holding_price_id`).
- **Import sao kê ngân hàng (map cột linh hoạt)**: trang Import thêm chế độ "Sao kê ngân hàng" — tải CSV bất kỳ ngân hàng → chọn tài khoản đích + **tự map cột** (Ngày; Số tiền 1 cột có dấu HOẶC 2 cột Nợ/Có; Nội dung), có preview. Parser số linh hoạt (`1.000.000`/`1,000,000`/`(500,000)` âm) và ngày `DD/MM/YYYY`·`YYYY-MM-DD`·`MM/DD/YYYY`. `src/lib/importBank.ts` (`parseAmount`/`parseFlexibleDate`/`validateBankRows`, +12 test). Giữ nguyên chế độ "Định dạng app".
...
### Technical
- **Verify Docker end-to-end thật** (`docker compose up --build`): Postgres healthy → `migrate` áp dụng đủ 14 migration + seed (kiểm cả DB cũ lẫn fresh volume `down -v`) → `web` phục vụ (`/`→/login, `/categories` query Postgres trả 200) → `cron` up. Lưu ý đặt `DB_PORT`/`WEB_PORT` khác nếu cổng 5432/3000 đã dùng.
- **Ngày-chỉ → `@db.Date` (UTC, hết lệch múi giờ)**: 8 cột ngày (`Transaction.date`, `Debt.startDate`, `DebtPayment.date`, `Goal.targetDate`, `TripExpense.date`, `TripSettlement.date`, `RecurringTransaction.startDate/endDate`) chuyển sang `date` Postgres (migration `date_only`, cast an toàn). Helper `src/lib/dateOnly.ts` (`parseDateInput`/`monthStartUTC`/`utcToday`, +6 test); mọi chỗ ghi, lọc tháng (Dashboard/Báo cáo/Ngân sách), nhóm tháng, `formatDate`, `addMonths`, `nextOccurrence`, `monthsUntil` dùng UTC nhất quán. Verify runtime: lọc biên tháng + hiển thị ngày không lệch. `nextRun` giữ timestamp (cursor lịch nội bộ).
- **Lint + test UI**: ESLint flat config (`eslint.config.mjs` — `next/core-web-vitals` + `next/typescript`), `npm run lint` → `eslint .` chạy sạch (sửa vài unused import / unescaped quote / prefer-const). Hạ tầng test component: vitest + `@vitejs/plugin-react` + jsdom + Testing Library (`src/test/setup.ts` mock matchMedia); test mẫu `ThemeToggle.test.tsx`. Tổng 73 test.
