...
2. **CoinGecko ID** (tùy chọn, chỉ cho tiền ảo): nếu coin của bạn **không nằm sẵn** trong danh sách hỗ trợ, nhập đúng **id của CoinGecko** (vd `bitcoin`, `the-open-network` — xem trên trang coin ở coingecko.com) để app cập nhật được giá. Bỏ trống = app tự khớp theo Mã. (~50 mã phổ biến đã hỗ trợ sẵn.)
3. Nhập **giá hiện tại** ở ô từng dòng rồi bấm ↻ để cập nhật thủ công.
4. Hoặc bấm **↻ Cập nhật giá** để tự lấy giá: tiền ảo (CoinGecko), cổ phiếu VN (VNDirect). Cột Lãi/Lỗ và % cập nhật theo.
5. Mục **Lịch sử giá** ở cuối trang: chọn mã để xem biểu đồ đường giá theo thời gian (cần ≥ 2 lần cập nhật giá).
...
2. Mở nhóm → thêm **thành viên** (chỉ cần tên). Bấm **☆** cạnh tên để đánh dấu thành viên đó là **bạn** (hiện badge "Tôi") — dùng để liên kết với tài khoản cá nhân.
3. **Thêm chi phí**: nhập **Nội dung** (bắt buộc), chọn **Người trả**, **Ngày**, **Tiền tệ** (mặc định VND), chọn cách chia:
...
4. **Bạn tự ứng tiền** → liên kết giao dịch: nếu người trả là **bạn** (đã đánh dấu "Tôi"), form hiện ô **trừ vào tài khoản** — chọn một hoặc **nhiều** tài khoản (tiền mặt/bank…) và số tiền mỗi nguồn. Mỗi nguồn tạo **một giao dịch chi**, trừ vào số dư, ghi **toàn bộ** số bạn ứng. Để trống nếu không muốn ghi giao dịch. Xoá khoản chi sẽ hoàn lại số dư.
5. **Đa tiền tệ**: mỗi khoản chi có thể khác tiền tệ (VD đi nước ngoài: vài khoản USD, vài khoản VND). Danh sách hiện số tiền gốc kèm "≈ VND". **Báo cáo phân tích** và **Phương án thanh toán** quy đổi tất cả về **VND** theo tỷ giá ở trang **Tỷ giá** — nhớ cập nhật tỷ giá cho đúng (thiếu tỷ giá thì khoản đó tạm coi như VND).
6. Xem **Báo cáo phân tích** (mỗi người đã trả / phải chịu / chênh lệch) và **Phương án thanh toán** (ai chuyển cho ai bao nhiêu — tối thiểu số lần chuyển).
7. **Ghi nhận thanh toán** (tổng kết): mỗi dòng trong Phương án thanh toán có nút **Ghi nhận** → đánh dấu đã trả (cập nhật cân bằng). Nếu khoản liên quan tới **bạn**: chọn tài khoản để tạo giao dịch **thu** (khi người khác trả bạn) hoặc **chi** (khi bạn trả người khác) — số tiền tự quy đổi về tiền tệ tài khoản. Mục **Đã ghi nhận thanh toán** liệt kê các khoản đã ghi + nút **Hoàn tác** (gỡ giao dịch & hoàn số dư).
...
- **Import CSV** (trang **Import**) có 2 chế độ:
  - **Sao kê ngân hàng (map cột)**: tải file CSV của **bất kỳ ngân hàng nào** → chọn **tài khoản đích** rồi tự **map cột**: cột **Ngày** (chọn định dạng DD/MM/YYYY…), **Số tiền** (chọn 1 cột có dấu: âm = chi, dương = thu; HOẶC 2 cột **Nợ/Có**), **Nội dung** (tùy chọn). Có bảng **preview**. App tự đọc số kiểu `1.000.000` / `1,000,000` và ngày đa định dạng.
  - **Định dạng app**: file đúng cột với export (`date, type, amount, currency, account, to_account, category, note`). `type` chấp nhận Thu/Chi/Chuyển hoặc INCOME/EXPENSE/TRANSFER; `account`/`category` khớp theo tên. Mẹo: xuất CSV trước để có mẫu.
  - Cả hai: giao dịch import **cập nhật số dư**; dòng lỗi được **bỏ qua và liệt kê**.
...
- **Bấm Cập nhật giá nhưng có mã bị "bỏ qua"**: mã tiền ảo chưa được hỗ trợ sẵn → nhập **CoinGecko ID** cho mã đó (xem mục Đầu tư), hoặc mã cổ phiếu sai/không có dữ liệu. Kiểm tra lại mã.
